...
extension LaunchesViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "LaunchesTableViewCell", for: indexPath) as?
                LaunchesTableViewCell else { return UITableViewCell() }
        cell.configure(name: launchesPresenter?.launchesRocket?[indexPath.row].name ?? "none",
                       date: launchesPresenter?.convertDate(dateLaunch: launchesPresenter?
                        .launchesRocket?[indexPath.row].date_utc ?? "none") ?? "none",
